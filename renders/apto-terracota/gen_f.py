...
# ///
"""Versão F — pintura bicolor orgânica (teto+topo terroso muted, base creme, curvas discretas).
Sem tapete; pendente de palha sobre a mesa. Via OpenRouter Flash com max_tokens capado."""
...
# Técnica de pintura inspirada nas referências, porém contida (sem exageros de curva nem de cor).
TECH_F = ("PINTURA BICOLOR ORGÂNICA (estilo das referências, porém CONTIDO): pinte o TETO e a FAIXA SUPERIOR "
          "das paredes (aprox. o terço de cima) numa cor TERROSA QUENTE e SUAVE — terracota/argila amendoada, "
          "MUTED e dessaturada (nada berrante nem escuro demais) — de modo que a cor pareça 'escorrer' do teto "
          "para o alto das paredes. A parte INFERIOR das paredes (os dois terços de baixo) fica num tom CLARO, "
          "neutro e quente (branco-cru/creme/off-white levemente amendoado). A transição entre a zona colorida "
          "de cima e a clara de baixo é feita com uma linha horizontal suave e CANTOS APENAS LEVEMENTE "
          "ARREDONDADOS — curvas DISCRETAS e elegantes, SEM EXAGEROS (nada de curvas grandes, dramáticas ou "
          "chamativas). Resultado sóbrio, harmônico, aconchegante; paleta terrosa suave estilo boho-minimalista.")
MOOD = ("CLIMA: intimista, aconchegante e confortável. ILUMINAÇÃO INDIRETA e quente (fitas de LED discretas, "
        "luminárias, spots suaves). MUITO IMPORTANTE: apesar da luz indireta, a imagem deve ficar BEM ILUMINADA "
        "e clara — NÃO escura nem subexposta. Fotorrealista, alta qualidade, estilo foto de revista.")
...
PENDANT = ("ADIÇÃO: instale um PENDENTE (luminária de teto) centralizado SOBRE a MESA de jantar — cúpula de "
           ...
           "fio a partir do teto, na altura típica de pendente de jantar. Estilo idêntico ao pendente de fibra "
           "natural trançada da referência (a cúpula de palha bege).")
...
# (arquivo, aspecto, view, tem_tv, tem_mesa_para_pendente)
PHOTOS = {
...
def build(view, has_tv, has_table):
    parts = ["INSTRUÇÃO DE EDIÇÃO — REDECORE este ambiente real a partir da fotografia.",
             CAMERA, LAYOUT, TECH_F, MOOD, NO_RUG]
    if has_tv:
        parts.append(RACK)
    if has_table:
...
def main():
    ...
    jobs = [(f"{pk}_F-bicolor-organica", f, a, v, tv, tb,
             os.path.join(outdir, f"{pk}_F-bicolor-organica.png"))
